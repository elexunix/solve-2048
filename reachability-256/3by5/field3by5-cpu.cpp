...
int main() {
    ...
    std::cout << "start with layer (default " << 3 * 5 * win_const / 2 << "): ";
    int init_layer_sum;
    std::cin >> init_layer_sum;
    for (int sum = init_layer_sum; sum >= 0; sum -= 2) {
        report_start("processing layer with sum " + itos(sum));
        std::cout << " sizes in bytes of current, current+2, current+4: " << (index_dp[15][sum] / 8 + 1 >> 20) << "M "
                << (index_dp[15][sum + 2] / 8 + 1 >> 20) << "M " << (index_dp[15][sum + 4] / 8 + 1 >> 20) << "M\n";
        ...
        if (sum + 2 < layers_cnt) {
            ...
            read_arr_from_disk(index_dp[15][sum + 2] / 8 + 1, user_turn_positions[sum + 2], "arrays3by5/ulayer" + itos(sum + 2, 4) + ".dat");
        }
        if (sum + 4 < layers_cnt) {
            user_turn_positions[sum + 4] = new unsigned char[index_dp[15][sum + 4] / 8 + 1];
            read_arr_from_disk(index_dp[15][sum + 4] / 8 + 1, user_turn_positions[sum + 4], "arrays3by5/ulayer" + itos(sum + 4, 4) + ".dat");
        }
        ...
        for (int i = 0; i < CPU_concurrency; ++i)
            kernels[i] = std::thread(process_user_layer, user_turn_positions, hater_turn_positions, sum, total_cnt / CPU_concurrency * i / 8 * 8,
                    i == CPU_concurrency - 1 ? total_cnt : total_cnt / CPU_concurrency * (i + 1) / 8 * 8);
        // write_arr_to_disk(index_dp[15][sum] / 8 + 1, hater_turn_positions[sum], "arrays3by5/hlayer" + itos(sum, 4) + ".dat");
        for (int i = 0; i < CPU_concurrency; ++i)
            kernels[i].join();
        delete[] hater_turn_positions[sum];
        write_arr_to_disk(index_dp[15][sum] / 8 + 1, user_turn_positions[sum], "arrays3by5/ulayer" + itos(sum, 4) + ".dat");
        // system("gzip arrays3by5/ulayer" + itos(sum, 4) + ".dat &");  -- this is impossible, because it will not allow this program to read this layer later
